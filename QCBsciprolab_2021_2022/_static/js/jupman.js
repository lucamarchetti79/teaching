...
var jupman = {
    ...
    */
    ...
    /**  
    */
    _setSlideColor : function(el, textColor) {
        el.style.color = textColor;
        Array.from(el.children).forEach((e) => {jupman._setSlideColor(e, textColor);});
    },
    
    /**  
    */
    setSlide : function(backgroundImg, textColor) {
        // Taken inspiration from https://alessandrobruni.name/software/2018/03/23/jupyter-slide-background.html
        
        if (backgroundImg.src){
            console.log('the image is bla ', backgroundImg);
            var src = backgroundImg.src; 
            console.log('the src is ', src);        
            //var section = img jupman._findAncestor(img, "section");
            var section = backgroundImg;
            while ((section = section.parentElement) && section.nodeName.toLowerCase() !== "section");            
            if (section) {
                section.setAttribute("data-background-image", src);
                if (textColor) {
                    jupman._setSlideColor(section, textColor);                    
                }
            }
...
$
    ...
    */
    ...
    init : function(){
       ...
       if (!(typeof JUPMAN_IN_SLIDES === "undefined")){
           console.log("SLIDES ENVIRONMENT!");  
       } else if (typeof JUPMAN_IN_JUPYTER === "undefined" || !JUPMAN_IN_JUPYTER ){
           jupman.initWebsite();
       } else {
           jupman.initJupyter();
       }
